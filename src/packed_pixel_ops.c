...
static int packed_offset(enum imgFormat fmt, int component_bit) {
	...
	case IMG_FMT_COMPONENT_RED:
		colour = 0;
		break;
	case IMG_FMT_COMPONENT_GREEN:
		colour = 1;
		break;
	case IMG_FMT_COMPONENT_BLUE:
		colour = 2;
		break;
	case IMG_FMT_COMPONENT_ALPHA:
		colour = 3;
		break;
	}
...
static void write_packed5(struct imgData *dst, int offset, float src) {
	...
	unsigned short s = *(unsigned short *) dst->channel[0];

	unsigned short dsrc = (unsigned short) (src * 31.0f);

	static const unsigned short shifts[] = { 10, 5, 0 };
...
static void write_packed8(struct imgData *dst, int offset, float src) {
	...
	(*(((unsigned char*) dst->channel[0]) + offset)) = (unsigned char) (src
			* 255.0f);
}

static float read_packed16(const struct imgData *data, int offset) {

	return ((float) (*(((unsigned short *) data->channel[0]) + offset)))
			/ 65535.0f;
}

static void write_packed16(struct imgData *dst, int offset, float src) {

	(*(((unsigned short*) dst->channel[0]) + offset)) = (unsigned char) (src
			* 65535.0f);
}
...
static read_fnptr get_read_function(enum imgFormat fmt) {
	...
	if (fmt & IMG_FMT_COMPONENT_PACKED) {
		...
		{
			// read 4-4-4-4
			int mask = (IMG_FMT_COMPONENT_PACKED16 | IMG_FMT_COMPONENT_RED
					| IMG_FMT_COMPONENT_GREEN | IMG_FMT_COMPONENT_BLUE
					| IMG_FMT_COMPONENT_ALPHA);
			if ((fmt & mask) == mask) {
...
static write_fnptr get_write_function(enum imgFormat fmt) {
	...
	if (fmt & IMG_FMT_COMPONENT_PACKED) {
		...
		if (fmt & IMG_FMT_COMPONENT_PACKED16) {
			// write 4-4-4-4
			int mask = (IMG_FMT_COMPONENT_RED | IMG_FMT_COMPONENT_GREEN
					| IMG_FMT_COMPONENT_BLUE | IMG_FMT_COMPONENT_ALPHA);
...
struct imgPixel imgReadPacked(const struct imgImage *img, int x, int y) {
	...
	if (img->format & IMG_FMT_COMPONENT_RED)
		pix.red = read_px(&imgData,
				packed_offset(img->format, IMG_FMT_COMPONENT_RED));
	if (img->format & IMG_FMT_COMPONENT_GREEN)
		pix.green = read_px(&imgData,
				packed_offset(img->format, IMG_FMT_COMPONENT_GREEN));
	if (img->format & IMG_FMT_COMPONENT_BLUE)
		pix.blue = read_px(&imgData,
				packed_offset(img->format, IMG_FMT_COMPONENT_BLUE));
	if (img->format & IMG_FMT_COMPONENT_ALPHA)
		pix.alpha = read_px(&imgData,
				packed_offset(img->format, IMG_FMT_COMPONENT_ALPHA));
...
void imgWritePacked(struct imgImage *img, int x, int y, struct imgPixel pix) {
	...
	if (img->format & IMG_FMT_COMPONENT_RED)
		write_px(&imgData, packed_offset(img->format, IMG_FMT_COMPONENT_RED),
				pix.red);
	if (img->format & IMG_FMT_COMPONENT_GREEN)
		write_px(&imgData, packed_offset(img->format, IMG_FMT_COMPONENT_GREEN),
				pix.green);
	if (img->format & IMG_FMT_COMPONENT_BLUE)
		write_px(&imgData, packed_offset(img->format, IMG_FMT_COMPONENT_BLUE),
				pix.blue);
	if (img->format & IMG_FMT_COMPONENT_ALPHA)
		write_px(&imgData, packed_offset(img->format, IMG_FMT_COMPONENT_ALPHA),
				pix.alpha);
