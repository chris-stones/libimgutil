...
{ { 0.299f, 0.587f, 0.114f }, { -0.169f, -0.331f, 0.500f }, { 0.500f, -0.419f,
		-0.081f } } };

static const struct mat3x3 ycbcr2rgb_mat = {

{ { 1.000f, 0.000f, 1.400f }, { 1.000f, -0.343f, -0.711f }, { 1.000f, 1.765f,
		0.000f }, } };

static void matmult(const struct mat3x3 *m33, const struct mat3x1 *m31,
		struct mat3x1 *out) {
...
static int planar_channel(enum imgFormat fmt, int component_bit) {
	...
	case IMG_FMT_COMPONENT_Y:
		colour = 0;
		break;
	case IMG_FMT_COMPONENT_CB:
		colour = 1;
		break;
	case IMG_FMT_COMPONENT_CR:
		colour = 2;
		break;
	...
	default:
		assert(0);
		break;
	case IMG_FMT_COMPONENT_YCBCRA:
		order = 0;
		break;
	case IMG_FMT_COMPONENT_YCRCBA:
		order = 1;
		break;
	}
...
static float read_planar8(const struct imgData *data, int channel) {
	...
	return ((float) (*(((unsigned char *) data->channel[channel]) + 0)))
			/ 255.0f;
}

static void write_planar8(struct imgData *dst, int channel, float src) {

	(*(((unsigned char*) dst->channel[channel]) + 0)) = (unsigned char) (src
			* 255.0f);
}
...
struct imgPixel imgReadPlanar(const struct imgImage *img, int x, int y) {
	...
	if (img->format & IMG_FMT_COMPONENT_CB)
		Cb = read_px(&imgData,
				planar_channel(img->format, IMG_FMT_COMPONENT_CB));
	if (img->format & IMG_FMT_COMPONENT_CR)
		Cr = read_px(&imgData,
				planar_channel(img->format, IMG_FMT_COMPONENT_CR));
	if (img->format & IMG_FMT_COMPONENT_ALPHA)
		pix.alpha = read_px(&imgData,
				planar_channel(img->format, IMG_FMT_COMPONENT_ALPHA));
...
void imgWritePlanar(struct imgImage *img, int x, int y, struct imgPixel pix) {
	...
	if (img->format & IMG_FMT_COMPONENT_CB)
		write_px(&imgData, planar_channel(img->format, IMG_FMT_COMPONENT_CB),
				Cb);
	if (img->format & IMG_FMT_COMPONENT_CR)
		write_px(&imgData, planar_channel(img->format, IMG_FMT_COMPONENT_CR),
				Cr);
	if (img->format & IMG_FMT_COMPONENT_ALPHA)
		write_px(&imgData, planar_channel(img->format, IMG_FMT_COMPONENT_ALPHA),
				pix.alpha);
}
